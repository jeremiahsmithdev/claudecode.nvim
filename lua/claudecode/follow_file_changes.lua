...
--- Find a suitable main editor window to open files in.
...
function M.reload_file_buffers(file_path, original_cursor_pos)
  logger.debug("follow_file_changes", "Reloading buffers for file:", file_path, original_cursor_pos and "(restoring cursor)" or "")
  ...
  logger.debug("follow_file_changes", "Completed buffer reload - reloaded", reloaded_count, "buffers for file:", file_path)
end
...
function M.navigate_to_file_after_change(file_path, cursor_pos)
  ...
  if target_win then
    ...
    logger.debug("follow_file_changes", "  Before navigation - win:", current_win, "buf:", current_buf, "file:", current_file)
