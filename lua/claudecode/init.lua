...
function M._broadcast_at_mention(file_path, start_line, end_line)
  ...
  -- Use main branch format that Claude expects
  local params = {
  ...
  logger.info("init", "Broadcasting at_mentioned with main branch format:", vim.inspect(params))
  local broadcast_success = server.broadcast("at_mentioned", params)
  logger.info("init", "Broadcast result:", broadcast_success)
